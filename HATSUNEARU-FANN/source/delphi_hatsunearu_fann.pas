...
function fann_relu_real(sum: fann_type): fann_type; inline;
// #define fann_relu_derive(steepness, sum) (sum > 0.0 ? steepness : 0.0)
function fann_relu_derive(steepness, sum: fann_type): fann_type; inline;

// #define fann_leaky_relu_real(sum) (sum > 0.0 ? sum : (sum / 100.0))
function fann_leaky_relu_real(sum: fann_type): fann_type; inline;
// #define fann_leaky_relu_derive(steepness, sum) (sum > 0.0 ? steepness : (steepness / 100.0))
function fann_leaky_relu_derive(steepness, sum: fann_type): fann_type; inline;
...
function fann_leaky_relu_derive;
begin
  if sum > 0 then
    result := steepness
  else
    result := steepness / 100;
end;
...
begin
  case activation_function of
    ...
    FANN_GAUSSIAN_STEPWISE:
      result := 0;
    FANN_RELU:
      result := fann_relu_real(value);
    FANN_LEAKY_RELU:
      result := fann_leaky_relu_real(value);
  else
    result := 0;
  end;
end;
